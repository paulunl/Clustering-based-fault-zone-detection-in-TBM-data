...
df = pd.read_parquet(r'02_processed_data\02_TBMdata_BBT_S_preprocessed.gzip.gzip')
...
def plot(df, FROM, TO, WINDOW):
        ...
        ######################################################################
        # plot thrust force
        fig, (ax4, ax5, ax1, ax2, ax3) = plt.subplots(nrows=5, ncols=1, figsize=(12, 8))
        ...
        ax1.set_ylabel('Advance Force [kN]', rotation=90)
        ax1.grid(alpha=0.5)
        ...
        ax3.set_ylim(0, 5)
        ax3.set_ylabel('spec. penetration\n'
        ...
        ax5.set_ylim(0, 4)
        ax5.set_ylabel('Main drive torque [MNm]', rotation=90)
